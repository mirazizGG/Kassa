...
- Daily jobs (`BOT_REPORT_TIME`, `BACKUP_TIME`, `DEBT_REMINDER_TIME`) run once per shop day, catching up the same day if the PC was off. Alerts (below-price sale, refund, shift closed with a discrepancy, low stock) are found by polling cursors, not audit text; on first run cursors start at "now" so history is not replayed.
