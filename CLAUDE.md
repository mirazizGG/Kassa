...
- Menus follow the old Python bot: client (balance, bonus), staff (clock in/out → `attendance`), admin (daily report, who's working, Excel data, broadcast). Registration matches the phone's last 9 digits against `employees.phone` first, then `clients.phone`; a contact is only accepted if `contact.user_id` equals the sender.
