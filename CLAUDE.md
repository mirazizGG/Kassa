...
**Primary admin lock:** the account named `PRIMARY_ADMIN_USERNAME` ([core.py](backend/core.py), `miraziz`) cannot be edited or deleted through the API — `PATCH`/`DELETE /auth/employees/{id}` refuse it (403/400) and the frontend hides those actions for that row. Change its password only on the server with `python reset_admin.py "new-password"` (prompts if no arg; never writes the password to disk). Other admin accounts can be freely added, edited, and deleted by an admin.
