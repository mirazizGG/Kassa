...
`GET /settings/backup/download` streams a fresh dump (`?uploads=1` → the invoice archive, 204 if none). It accepts an admin JWT or an `X-Bot-Key` header equal to `BOT_API_KEY` (≥ 32 chars, `hash_equals`; only failed keys count toward a 10/hour per-IP limit, so the right key is never locked out). The bot relay uses it for the nightly backup.

### Telegram bot

The logic lives in [app/Bot.php](php/app/Bot.php) and runs in one of two modes:

- **Relay (the intended setup).** The shop PC runs [bin/bot.php](php/bin/bot.php) with `KASSA_API_URL` + `BOT_API_KEY` and **no database credentials**. It long-polls Telegram, forwards each update to `POST /bot/update`, calls `POST /bot/tick` every 30 s, and executes the actions the server returns (`send`, `document` with base64 bytes, `copy` for broadcasts, `backup` → it downloads `/settings/backup/download` and sends the file). On the server, [routes/bot.php](php/app/routes/bot.php) calls `Telegram::collect()`, so every `Telegram::send/sendDocument/copy` in `Bot.php` is queued as an action instead of hitting Telegram — the server needs no bot token. Schedule times and `TELEGRAM_ADMIN_CHAT_ID` are read from the **server's** `.env`. Conversation steps persist in `logs/bot-conv.json`, schedule/alert state in `logs/bot-state.json` (server side). If `/bot/update` fails the relay keeps the Telegram offset and retries, dropping an update only after 5 failures.
- **Direct.** `KASSA_API_URL` empty: `bin/bot.php` connects via `DATABASE_URL` and runs the same functions itself (for a till that hosts its own database).

`BOT_API_KEY` (≥ 32 chars, `X-Bot-Key` header, `hash_equals`, only failures rate-limited 10/hour/IP) is effectively the bot's identity — whoever has it can act as any Telegram user through `/bot/update`.

- Menus follow the old Python bot: client (balance, bonus), staff (clock in/out → `attendance`), admin (daily report, who's working, Excel data, broadcast). Registration matches the phone's last 9 digits against `employees.phone` first (`employee_by_phone()`), then `clients.phone`; a contact is only accepted if `contact.user_id` equals the sender. On `/start`, a Telegram user linked as a client is re-checked: if their phone now matches an employee with no `telegram_id`, the link moves to the employee (covers staff who joined the bot before the admin filled their phone). An employee already linked to another account is never taken over.
- Daily jobs (`BOT_REPORT_TIME`, `BACKUP_TIME`, `DEBT_REMINDER_TIME`) run once per shop day, catching up the same day if the PC was off. The only instant alert is a shift closed with a cash discrepancy (a `closed_at` cursor that starts at "now" on first run). Refund, below-price-sale and low-stock alerts were removed on request — the owner checks those on the site.
- [php/bot-windows/](php/bot-windows/) installs the relay: `ORNATISH.bat` → `install.ps1` downloads portable PHP into `runtime/`, runs `bin/bot-check.php` (in relay mode it calls `GET /bot/ping`), and registers the "Kassa bot" scheduled task (at startup as SYSTEM when elevated) running `start-bot.ps1`, which restarts the bot when it exits. It uses `Start-Process` with redirected output: `cmd /c` quoting broke on paths with spaces, and PowerShell 5.1 `*>>` wrote UTF-16. The shipped zip (`kassa-bot-*.zip`, gitignored because it contains `.env`) is `app/` + `bin/` + those scripts at the root. `logs/bot.lock` keeps one instance — two pollers on one token get Telegram 409.
- `BOT_DRY_RUN=true` logs messages instead of sending them; `define('BOT_NO_LOOP', true)` before requiring `bin/bot.php` loads the functions without the loop (PHP hoists the relay helpers too).
- On the CLI, `Db::pdo()` throws on a failed connection instead of `Http::fail`, which printed JSON and exited 0; `Db::reset()` drops a stale connection. `bot_log()` echoes only on the CLI so it never corrupts a JSON response.
