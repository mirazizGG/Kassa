...
const Layout = () => {
  ...
  const navItems = [
    ...
    ...(role === "admin"
      ? [{ path: "/settings", label: "Sozlamalar", icon: UserCog }]
      : []),
  ];
  ...
  return (
    <div className="flex h-screen w-full bg-background transition-colors duration-300">
      ...
      {role !== "cashier" && (
        ...
        >
          ...
          <div className="mt-auto p-4 border-t border-border space-y-4">
            {/* User Profile Info */}
            <div
              className={`flex items-center gap-3 px-2 py-2 rounded-xl bg-muted/50 border border-border/50 ${!isSidebarOpen && "justify-center px-0"}`}
            >
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden h-full bg-secondary/20">
        <div
          className={`flex-1 overflow-auto ${location.pathname === "/pos" ? "p-0" : "p-4 md:p-8"} ${role === "cashier" ? "pb-20" : ""}`}
        >
          <div
        ...
        {role === "cashier" && (
          <div className="fixed bottom-0 left-0 right-0 h-16 bg-card border-t border-border flex items-center justify-around z-50 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
            <NavLink
              to="/pos"
              className={({ isActive }) =>
                `flex flex-col items-center justify-center w-full h-full gap-1 ${isActive ? "text-primary" : "text-muted-foreground hover:text-foreground"}`
              }
            >
            ...
            <div className="flex flex-col items-center justify-center w-full h-full gap-1">
              <ModeToggle />
              <span className="text-xs font-medium">Rejim</span>
            </div>
            <button
              onClick={handleLogout}
              className="flex flex-col items-center justify-center w-full h-full gap-1 text-destructive hover:bg-destructive/10"
            >
