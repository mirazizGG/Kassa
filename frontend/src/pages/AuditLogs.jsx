...
const formatAuditDate = (value, options) =>
  new Intl.DateTimeFormat("uz-UZ", {
    timeZone: TASHKENT_TIME_ZONE,
    ...options,
  }).format(toAuditDate(value));
...
const AuditLogs = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `audit_${format(new Date(), "yyyyMMdd")}.xlsx`,
      );
  ...
  const getActionConfig = (action) => {
    const configs = {
      ...
      SMENA_OCHILDI: {
        icon: Calendar,
        color: "bg-blue-400/10 text-blue-500 border-blue-200",
        label: "Smena Ochiq",
      },
      SMENA_YOPILDI: {
        icon: Calendar,
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground flex items-center gap-2">
              ...
              Tizimdagi barcha muhim amallar va o'zgarishlar tarixi (Real vaqt
              rejimida)
            </p>
          </div>
        ...
        <CardHeader className="border-b bg-muted/20 pb-4">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <CardDescription>
                Oxirgi bajarilgan amallar ro'yxati
              </CardDescription>
            </div>
            ...
            <Table>
              <TableHeader className="bg-muted/10">
                <TableRow className="hover:bg-transparent border-none">
                  <TableHead className="w-[200px] pl-6 h-12">
                    Mas'ul xodim
                  </TableHead>
                  <TableHead className="w-[180px] h-12">Amal turi</TableHead>
                  <TableHead className="min-w-[300px] h-12">
                    Batafsil ma'lumot
                  </TableHead>
        ...
        <DialogContent className="sm:max-w-[500px] p-0 overflow-hidden bg-background/90 backdrop-blur-2xl border-primary/20 shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5 p-3 rounded-xl bg-muted/30 border border-border/50">
                ...
                <p className="text-sm font-bold">
                  {selectedLog?.user?.full_name || selectedLog?.user?.username}
                </p>
              </div>
              <div className="space-y-1.5 p-3 rounded-xl bg-muted/30 border border-border/50">
                <span className="text-[10px] font-bold text-muted-foreground uppercase flex items-center gap-1">
          ...
          <div className="p-4 bg-muted/10 border-t border-border/50 text-center">
            <p className="text-[10px] text-muted-foreground font-medium">
              Ushbu amal tizim tomonidan avtomatik qayd etilgan va o'zgartirib
              bo'lmaydi.
            </p>
          </div>
        </DialogContent>
