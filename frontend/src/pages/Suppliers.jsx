...
const Suppliers = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="rounded-2xl border bg-gradient-to-r from-primary/10 via-background to-background p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">
            Firmalar (Postavshiklar)
          </h1>
          <p className="text-muted-foreground">
            Kirimlar, to'lovlar va qarzlar nazorati
          </p>
        </div>
          ...
          <DialogHeader className="p-6 pb-2 border-b">
            <div className="flex justify-between items-start">
              <div>
                ...
                <DialogDescription>
                  To'liq hisob-kitoblar tarixi
                </DialogDescription>
              </div>
              <Badge
                variant={
                  selectedSupplier?.balance > 0 ? "destructive" : "secondary"
                }
                  ...
                  >
                    ...
                    {item.image && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="shrink-0 bg-white border p-1 rounded-lg hover:shadow-md transition-all"
                      >
      ...
      <Dialog open={isAddSupplyOpen} onOpenChange={setIsAddSupplyOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Firma tomonidan kelgan nakladnoyni kiriting.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleAddSupply} className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <ScrollArea className="h-[150px] border rounded-md p-2">
                {suppliers.filter((s) =>
                  s.name.toLowerCase().startsWith(supplySearch.toLowerCase()),
                ).length === 0 ? (
                  ...
                  <div className="space-y-1">
                    {suppliers
                      .filter((s) =>
                        s.name
                          .toLowerCase()
                          .startsWith(supplySearch.toLowerCase()),
                      )
                      .map((s) => (
                        <div
                          key={s.id}
              ...
              {newSupply.supplier_id && (
                <div className="text-[10px] text-emerald-600 font-bold mt-1">
                  Tanlandi:{" "}
                  {suppliers.find((s) => s.id == newSupply.supplier_id)?.name}
                </div>
              )}
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewSupply({ ...newSupply, note: e.target.value })
                }
                placeholder="Masalan: Shakar va yog' keldi"
              />
            </div>
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <Input
                  ...
                  accept="image/*"
                  onChange={(e) =>
                    setNewSupply({ ...newSupply, image: e.target.files[0] })
                  }
                  className="cursor-pointer"
                />
                {newSupply.image && (
                  <ImageIcon className="text-emerald-500 h-6 w-6" />
                )}
              </div>
            </div>
            <DialogFooter>
              <Button
                type="submit"
                disabled={addSupplyMutation.isPending}
                className="w-full"
              >
      ...
      <Dialog open={isAddPaymentOpen} onOpenChange={setIsAddPaymentOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Firmaga berilgan pulni qayd eting.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleAddPayment} className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <ScrollArea className="h-[150px] border rounded-md p-2">
                {suppliers.filter((s) =>
                  s.name.toLowerCase().startsWith(paymentSearch.toLowerCase()),
                ).length === 0 ? (
                  <div className="text-center py-4 text-muted-foreground italic text-sm">
                    Yo'q (Firma topilmadi)
                  </div>
                  ...
                  <div className="space-y-1">
                    {suppliers
                      .filter((s) =>
                        s.name
                          .toLowerCase()
                          .startsWith(paymentSearch.toLowerCase()),
                      )
                      .map((s) => (
                        <div
                          key={s.id}
              ...
              {newPayment.supplier_id && (
                <div className="text-[10px] text-emerald-600 font-bold mt-1">
                  Tanlandi:{" "}
                  {suppliers.find((s) => s.id == newPayment.supplier_id)?.name}
                </div>
              )}
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Masalan: Shakar pulidan 1 mln berildi"
              />
            </div>
            <DialogFooter>
              <Button
