 ...
 * ishga tushiradi va yiqilsa qayta ko'taradi.
 *
 * Nima qiladi:
 *   - Mijoz:  /start -> telefon -> ism; "Balansim", "Bonuslarim".
 *   - Xodim:  telefoni Xodimlar sahifasidagiga mos kelsa tanib oladi;
 *             "Ishga kelish" / "Ishdan ketish" (davomat).
 *   - Admin:  "Bugungi hisobot", "Kim ishda?", "Ma'lumotlar" (Excel),
 *             "Reklama yuborish".
 *   - Jadval: har kuni BOT_REPORT_TIME da kunlik hisobot, BACKUP_TIME da
 *             zahira nusxa, DEBT_REMINDER_TIME da qarz eslatmalari.
 *   - Ogohlantirish: vozvrat, kamomadli smena, narxdan past sotuv,
 *             tugayotgan mahsulot — adminga darhol.
 *
 * Holat (offset, oxirgi ishga tushgan sanalar, kursorlar) logs/bot-state.json
 * da saqlanadi, shuning uchun qayta ishga tushganda hech narsa ikki marta
 * yuborilmaydi va eski tarix "yangi" deb yog'dirilmaydi.
 */
...
/** @var array<int, array> Ro'yxatdan o'tish / reklama bosqichlari, chat bo'yicha */
$conv = [];
...
$state = state_load();
bot_log('bot ishga tushdi (baza: ' . Db::driver() . ')');
...
$lastAlerts = 0;
while (true) {
    // Avval jadval va ogohlantirishlar: getUpdates 25 soniyagacha kutib turadi.
    try {
        run_schedule($state);
        if (time() - $lastAlerts >= 30) {
            $lastAlerts = time();
            check_alerts($state);
            state_save($state);
        }
    ...
    try {
        ...
        foreach ($updates as $u) {
            ...
            try {
                ...
                bot_log('xabarni qayta ishlashda XATO: ' . $e->getMessage());
